...
declare @WorkignDaysAdjustment as int
...
BEGIN
		...
		declare @TotalGatePassOutMinutesPersonal as decimal (10,2)
		declare @TotalGatePassOutMinutesOfficial  as decimal (10,2)
		...
		ELSE
			...
			------- Conditional Query to pull Leaves from July 2021 onwards
				Select @TotalLeavesInAYear = count(1) from (Select Distinct ATTENDANCE_DATE from TAB_BIOMETRIC(NOLOCK) where WORKFORCE_ID = @WF_ID and STATUS_CODE='A' and ATTENDANCE_DATE>=@FinancialYearStartDate and ATTENDANCE_DATE < @STARTDATE and ATTENDANCE_DATE > '06/30/2021') as whoeyearleaves
				Select @LeaveCarryForward = LeaveCarryForward from TAB_LEAVE_CARRY_FWD where EmpCode=@EMP_ID
				Select @LeaveCarryForward = (@LeaveCarryForward + (DATEDIFF(MM,'07/01/2021',@STARTDATE))) - @TotalLeavesInAYear
			END
