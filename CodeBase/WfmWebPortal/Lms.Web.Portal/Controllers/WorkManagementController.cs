...
namespace Lms.Web.Portal.Controllers
{
    ...
    public class WorkManagementController : Controller
    {
        ...
        public WorkManagementController(IBaseBL baseBl)
        {
            this.baseBl = baseBl;
        }
        ...
        public ActionResult EditDailyWork(Guid? id)
        {
            ...
            //ViewBag.OperationName = new SelectList(baseBl.WorkforceBL.GetItemOperationsByItemId((metaData.ITEM_NAME)), "UNIQUE_OPERATION_ID", "OPERATION");
            ViewBag.ID = id.Value;
            if (id == null)
            {
        ...
        public JsonResult LoadWorkforce(Guid deptId, string query)
        {
            ...
            try
            {
                if (deptId == Guid.Empty)
                    result = baseBl.WorkforceBL.GetEmpAllItems().Where(x => x.EMP_NAME.ToUpper().Contains(query) || x.EMP_ID.ToUpper().Contains(query)).ToList();
                else
                {
                    AccountValidateUser_Result loggedin_user = Session["USER"] as AccountValidateUser_Result;
                    Guid companyId = loggedin_user.COMPANY_ID;
