...
function GetReport() {
    ...
    $.ajax(
        {
            type: 'Get',
            url: '/Reports/GetMissingPunchReport?BUILDING_ID=' + BUILDING_ID + '&DEPT_ID=' + DEPT_ID + '&SUBDEPT_ID=' + SUBDEPT_ID + '&WF_EMP_TYPE=' + WF_EMP_TYPE + '&EMP_NAME=' + EMP_NAME + '&MONTH_ID=' + MONTH_ID + '&YEAR_ID=' + YEAR_ID + '&EMPLOYMENT_TYPE=' + EMPLOYMENT_TYPE,
            beforeSend: function () {
                ...
                function (response) {
                    var html = '';
                        if (response != "") {
                            ...
                            html += '<th class="bold" colspan=' + col +' style="text-align: center; font-size: 1.7rem; ">Missing Punch Report</th>';
                            html += '</tr>';
                            html += '<tr>'
                            ...
                            html += '<td class="bold">Missing Date</td>'
                            html += '<td class="bold">Missing Days</td>'
                            ...
                            for (var i = 0; i < EmpList.length; i++) {
                               /* console.log(EmpList);*/
                                ...
                                if (data.EXIT_DATE == '1900-01-01') {
                                    html += '<td style="text-align:left"></td>'
                                    html += '<td style="text-align:left"></td>'
                                } else {
                                    html += '<td style="text-align:left">' + data.EXIT_DATE + '</td>'
                                    html += '<td style="text-align:left">' + data.ExitDays + '</td>'
                                }
