...
function GetReport() {
   ...
   /* var EMP_NAME = $('#WF_ID').val();*/
    ...
    if ((FROM_DATE.substring(3, 5)) != (TO_DATE.substring(3, 5))) {
        alert('Please select same month in From and To Date.');
        return;
    }
    ...
    $.ajax(
        {
            type: 'Get',
            url: '/Reports/GetMonthlyOTReport?BUILDING_ID=' + BUILDING_ID + '&DEPT_ID=' + DEPT_ID + '&SUBDEPT_ID=' + SUBDEPT_ID + '&WF_EMP_TYPE=' + WF_EMP_TYPE + '&EMP_NAME=' + EMP_NAME + '&FROM_DATE=' + FROM_DATE + '&TO_DATE=' + TO_DATE + '&EMPLOYMENT_TYPE=' + EMPLOYMENT_TYPE,
            beforeSend: function () {
                ...
                function (response) {
                    var html = '';
                        if (response != "") {
                            ...
                            var col = Number(DateList.length)+6;
                            ...
                            html += '<th class="bold" colspan=' + col+' style="text-align: center; font-size: 1.7rem; ">Monthly OT Report</th>';
                            html += '</tr>';
                            html += '<tr>'
                            ...
                            for (var i = 0; i < EmpList.length; i++) {
                               /* console.log(EmpList);*/
                                ...
                                for (var j = 0; j < data.AttendanceList.length; j++, sdate++) {
                                    ...
                                    TotalOT += Number(dt.OverTime);
                                }
                                html += '<td></td>'
                                html += '<td>' + conversion(TotalOT)+'</td>'
                                html += '</tr>'
                            }
                            html += '</tbody >'
                            html += '</table>'
